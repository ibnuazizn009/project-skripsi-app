                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="box">
                            ...
                            @can('create role')
                               <a href="/periode/add" class="btn btn-sm btn-success" style="padding: 8px;"><b> Tambah Data </b></a>
                            @endcan
                            </div><!-- /.box-header -->
                            <div class="box-body">
                                <div class="row">
                                        ...
                                        <table id="dtHorizontalVerticalExample"
                                            ...
                                            <thead style="font-size: 14px">
                                                <tr>
                                                    ...
                                                    <th>ID Periode</th>
                                                    <th>Periode</th>
                                                    <th>Status Aktif Periode</th>
                                                    <th>ID Semester</th>
                                                    <th>Tanggal Buka Pendaftaran</th>
                                                    <th>Tanggal Tutup Pendaftaran</th>
                                                    <th>Tanggal Tutup Perpanjangan</th>
                                                    <th>Tanggal Gladi</th>
                                                    <th>Tanggal Bagi Toga</th>
                                                    <th>Tanggal Pelaksanaan</th>
                                                    <th>Scan SK Wisuda</th>
                                                    <th>Username</th>
                                                    <th>Action</th>
                                                </tr>
                                            </thead>
                                            <tbody style="font-size: 14px">
                                                @foreach($periode as $pd)
                                                    <tr>
                                                        ...
                                                        <td>{{$pd->id_periode}}</td>
                                                        <td>{{$pd->periode}}</td>
                                                        <td>{{$pd->status_aktif_periode}}</td>
                                                        <td>{{$pd->id_semester}}</td>
                                                        <td>{{$pd->tgl_buka_daftar}}</td>
                                                        <td>{{$pd->tgl_tutup_daftar}}</td>
                                                        <td>{{$pd->tgl_tutup_perpanjangan}}</td>
                                                        <td>{{$pd->tgl_gladi}}</td>
                                                        <td>{{$pd->tgl_bagi_toga}}</td>
                                                        <td>{{$pd->tgl_pelaksanaan}}</td>
                                                        <td>{{$pd->scan_sk_wisuda}}</td>
                                                        <td>{{$pd->username}}</td>
                                                        <td class="">
                                                            @can(['update role', 'delete role'])
                                                            <form onsubmit=""
                                                                method="POST">
                                                                <a href="/periode/edit/{{$pd->idx}}" class="btn btn-sm btn-primary"><b>EDIT</b></a>
                                                                <a href="/periode/delete/{{$pd->idx}}" class="btn btn-sm btn-danger"><b>HAPUS</b></a>
